...
{{< alert >}}
**Podman and bootstrapping on mac os**: To run the bootstrap scripts validated patterns presumes that you are using `podman`. Using [brew](https://brew.sh/):
...
{{< alert >}}
**clusterGroup**: `clusterGroup` is a label used together with RHACM particularly for clusters beyond the first. Applying:
